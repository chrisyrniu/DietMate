...
raw_cv_path = "./data/Sina_Cookie_11_CSV.csv"
...
# Set the loops to find the best window sizes to extract the features which can produce the highest evaluation accuracy. Here the window sizes are fixed.
                ...
                for window_size5 in range(33, 34, 2):
                    for window_size6 in range(33, 34, 2):
                        for window_filter in range(51, 52, 2):
                            ...
                            # Eliminate the begining and the end after using the sliding windows.
                            ...
                            svmc = svm.SVC(degree = 7)
                            predicted = cross_val_predict(svmc, cv_x, cv_y, cv = 3)
                            predicted = pd.Series(predicted)

                            # Apply the mode filter to the estimated labels.
                            ...
                            # Find the highest evaluation accuracy and record the evaluation scores.
                            if metrics.accuracy_score(cv_y, predicted) > max_score:
                               ...
                               precision = metrics.precision_score(cv_y, predicted, average = 'weighted')
                               f_measure = metrics.f1_score(cv_y, predicted, average = 'weighted')
                               recall = metrics.recall_score(cv_y, predicted, average = 'weighted')
